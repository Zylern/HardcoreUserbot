# Ported from Javes and modified by Zylern
...
from userbot import CMD_HELP, ALIVE_NAME, bot, TEMP_DOWNLOAD_DIRECTORY, BOTLOG, BOTLOG_CHATID
...
async def vt(event):
    ...
    if a ==2:
		      return await event.edit("`You need to Update MastUserBot to use this command.......`")
    if not vscan:
...
async def vt(event):
    ...
    vscan = downloaded_file_name
    if a ==2:
		      return await rkp.edit("`You need to Update MastUserBot to use this command.......`")
    if not vscan:
...
async def _(event):
    ...
    async with bot.conversation(chat) as conv:
          try:        
              ...
              return await event.edit(f"**MastUserBot:**  {song2.message}")
          except:      
...
async def _(event):
    ...
    async with bot.conversation(chat) as conv:
          try:        
              ...
              await conv.send_message(reply_message)
              song2 = await conv.get_response()
              return await rkp.edit(f"**MastUserBot:**  {song2.message}")
          except:      
              return await event.reply(f"Please unblock @DrWebBot and try again")

                                

@register(outgoing=True, pattern="^.uscan(?: |$)(.*)")
async def _(event):
...
@borg.on(admin_cmd(pattern=f"uscan(?: |$)(.*)", allow_sudo=True))
async def _(event):
       rksong = event.pattern_match.group(1)
       if not rksong:
